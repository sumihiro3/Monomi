...
Monomi は [claude-release-workflow](https://github.com/sumihiro3/claude-release-workflow) テンプレート（エンジン＝プロジェクト中立な dynamic workflows。導入・更新手順は同リポジトリ README）を採用し、Claude Code の Workflow tool（dynamic workflows）による 7 ステップのリリースサイクルで開発する。プロジェクト固有の設定（検査コマンド・レビュー観点・同期先文書・自動化設定）は `.claude/workflow.config.json` に置き、テンプレートの版は同ファイルの `templateVersion` で追跡する（更新は `/update-release-workflow`）。本ドキュメントは、その汎用テンプレートを Monomi の実体（`.claude/workflows/`・`.claude/commands/`・`docs/releases/release-N/requirements.md`）に即して具体化した運用ガイドである。
...
| build  | `pnpm run build`        | `tsc -p tsconfig.json` ＋ `chmod +x dist/bin.js`          |
...
- `.claude/workflows/*.js` 自身を変更するリリースは、run-release を使わず、従来の段階実行（`implement-feature` → `review-changes` → `sync-docs` → `release-check` → コミット → PR）で行う。
- 理由: 無人実行中は、実装フェーズで書き換えた直後の未検証の新エンジンコードを、同一実行内のネスト `workflow()` 呼び出し（Gate2 再レビュー・doc 同期など）がそのまま実行してしまう自己変更ハザードが生じるため（`workflow()` の name/scriptPath 解決は呼び出し時点のファイルを読む）。
- 前例: `release-12-workflow-generalization`（ワークフローエンジンの汎用化）・`release-14-pipeline-performance`（本リリース。`review-changes.js`・`sync-docs.js`・`run-release.js` の性能改善）。いずれも従来の段階実行で実施した。
...
### npm 公開（publish）運用（release-17 以降）

`v*` タグの push が `.github/workflows/publish.yml` の publish ジョブを起動する。バージョン bump の直後の手動 push ステップ（上記）にそのまま接続する。

**release-22 以降**: publish ジョブは npm publish と同じタイミングで GitHub Releases も自動作成する（known-issues N5 対応）。ステップ順序は「test → build → CHANGELOG 該当版のノート抽出（fail fast）→ npm publish → Release 作成」。ノート抽出は `scripts/extract-changelog-notes.mjs` が `CHANGELOG.md` から今回 bump したバージョンの節（`## [x.y.z] - YYYY-MM-DD` 見出し配下の本文）を抽出して標準出力へ渡し、失敗時（該当バージョン節が無い・本文が空 等）は非0 終了でジョブ全体を止める。抽出結果は一時ファイルへリダイレクトし、`gh release create "${GITHUB_REF_NAME}" --notes-file <一時ファイル> --verify-tag`（既存 runner 同梱の `gh` を使用。新規サードパーティ Action は追加しない）で Release 本文として渡す。`--verify-tag` によりタグの実在を検証してから作成するため、タグとリリースの版ずれを防ぐ。ジョブに付与する権限は Release 作成に必要な `contents: write` のみ。

**初回のみ（`NPM_TOKEN` の発行・登録）**:

1. npm アカウントで granular access token を発行する（対象パッケージ: `monomi-cli` のみ・権限: publish に必要な最小範囲・有効期限を設定する）
2. 発行した token を GitHub Secrets へ登録する（対話プロンプトで貼り付ける。token をコマンド履歴・ドキュメント・コミットに残さないこと）:
...
1. `git checkout main && git pull` — main を最新化する
2. `git status` — working tree が clean であることを確認する（dirty な場合は `pnpm version` が失敗する。上記「working tree の状態確認」を参照）
3. `CHANGELOG.md` の `## [Unreleased]` セクションの内容を、これから bump する新バージョンの見出し（`## [x.y.z] - YYYY-MM-DD`）へ繰り上げ、`[Unreleased]` セクション自体は見出しだけ残して空にする。この変更を先に commit する（`pnpm version` 実行時に working tree が clean である必要があるため）:

   ```sh
   git add CHANGELOG.md
   git commit -m "docs: CHANGELOG を vX.Y.Z へ繰り上げ"
   ```

4. `pnpm version:patch`（または `pnpm version:minor` / `pnpm version:major`）— `package.json` の bump・commit・tag 作成（`preversion` で test が実行される）
5. `git push --follow-tags` — main と tag を push する。tag push が publish ワークフローのトリガーになる
6. GitHub Actions で publish ジョブが成功したことを確認する:

   ```sh
   gh run list --workflow=publish.yml --limit 1
   ```

7. 該当バージョンの GitHub Release が自動作成されたことを確認する（`x.y.z` は今回 bump したバージョン）:
...
8. npm への到達を確認する:
   ...
   ```sh
   npm view monomi-cli version
   ```

**公開直後の動作確認**（クリーン環境相当。別ディレクトリ／別マシンなどグローバル未導入の環境で行う）:
...
`monomi --version` の出力が bump 後のバージョンと一致することを確認する。
...
| [claude-release-workflow](https://github.com/sumihiro3/claude-release-workflow) | ワークフローテンプレートの正典リポジトリ（導入・更新手順は同 README）                                |
