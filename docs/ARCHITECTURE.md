...
- **`running_work`（release-16）**: 「実行中の作業名」。`null` または `{ "kind": "workflow" | "agent" | "skill", "name": "run-release" }`。`instance_id` 直下（`status`/`session` と同じ階層）に置く。`GET /api/v1/instances/:id`（`InstanceDetail`）も `InstanceStatusRow` を継承するため同じ形で含まれる。
  - 導出元は既存 `events` テーブルの `tool_name`/`tool_summary`（DB スキーマ変更なし）。代表 session の直近イベントを遡り、区切りイベント（`Stop`・`Notification(idle_prompt)`・`SessionEnd`・`UserPromptSubmit`）に当たるまでの区間内で、`PreToolUse(tool_name=Workflow)` があれば最新の `tool_summary` を `kind: "workflow"` として優先し（無ければ `Task`/`Agent`/`Skill` の最新を `kind: "agent"`/`"skill"`）返す。
  - **ACTIVE ゲート**: 代表 session の `status.raw_state` が `active` のときだけ上記の走査を行う。非 active（`approval_wait`/`next_wait`/closed 相当）は走査せず即 `null` を返す。上の例は `raw_state: "approval_wait"` のため `running_work` が `null` になっている（`Workflow` 実行中に承認待ちへ入っても、待機中は表示を消す。要件上「消灯は `Stop`/`idle`/`SessionEnd` のみ」という記述と「非 active なら null」という記述が承認待ち中に衝突するが、**後者（ACTIVE ゲート優先）で確定**しており実装・レビューで再度議論しない）。
  ...
  - 性能: ACTIVE ゲートにより非 active な instance では追加のイベント読み取りが発生しない（`buildRow` が既に算出済みの `StatusResult` を再利用するのみ）。一方 ACTIVE な instance では、`loadRunningWorkForCurrentRun()` が status 導出用の `loadEventsForCurrentRun()` とは別に代表 session のイベントを再度ページング読み込みする（区切り集合が異なるため単純な再利用はできない、上記）。ポーリング間隔（~2s）に対しては軽微（クエリ 1 回 ~1ms）だが、instance 数が増えるとポーリング 1 サイクルあたりの累積遅延が線形に伸びる、既知課題 P3（一覧生成の N+1）に隣接する新規の N+1 パターンであり、未解消のまま残す（`docs/known-issues.md` P8）。
...
- **スコープ外**: push 自動化、CHANGELOG.md 生成、npm registry への公開（`package.json` は `"private": true`）。
...
- CHANGELOG.md 自動生成、npm registry への公開、バージョン bump 後の `git push` 自動化（§13。手動ステップのまま）
