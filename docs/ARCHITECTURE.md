...
- **対応 OS**: macOS・WSL2・ネイティブ Linux（WezTerm 利用時のみ、release-28-wezterm-focus）。
  ...
  - **WSL2**: `$WEZTERM_PANE` を捕捉できれば WezTerm ペイン単位フォーカスを先に試行し、未検出（未設定・従来ユーザー）またはフォーカス失敗時は既存の Windows Terminal ウィンドウ前面化（タブ単位の特定はスコープ外）へフォールバックする（後方互換維持）。WSL2 で WezTerm フォーカスを使うには、利用者側で Windows 側 `.wezterm.lua` に `WSLENV` へ `WEZTERM_PANE` を追記する設定が前提（自動設定はしない設計判断、§14.5）。macOS と同様 `activate-pane` だけでは前面化されない見込みのため、`raiseWeztermWindowWsl`（`wezterm-gui` プロセスへ `SetForegroundWindow`）を追加実行する設計だが、**実 Windows/WSL2 環境での動作は未検証**（実機確認が必要、requirements.md 未解決事項）。
  - **ネイティブ Linux（X11/Wayland）**: release-23 まではスコープ外だったが、release-28-wezterm-focus で WezTerm 利用時のみ初めて対応した（`wezterm cli` 呼び出しのみで完結し X11/Wayland のウィンドウ操作 API に依存しないため）。WezTerm 以外のターミナルは引き続き未対応。**OS レベルのウィンドウ前面化は行わない**（X11/Wayland のウィンドウ操作 API への依存を避ける設計判断のため）。`activate-pane` によるペイン切替は行われるが、WezTerm ウィンドウ自体が他アプリより前面に出ることは保証しない best-effort（既知の制限、known-limitations.md 参照）。
...
- **`focus-service.ts`（ディスパッチ、release-28-wezterm-focus FR-04 で拡張）**: 手順は次のとおり。
  ...
  5. それ以外（ネイティブ Linux 等。release-28-wezterm-focus で新規サポート）は、`weztermPane` があれば `WeztermFocusStrategy`（`command: 'wezterm'`）を試行しその結果を返し、無ければ `unsupported_platform`。
...
| release-28-wezterm-focus                       | ターミナルフォーカス（`f` キー、§14）の対象へ WezTerm を追加（既知課題 U17 解消）。reporter が毎フックで `$WEZTERM_PANE` を捕捉し `terminal.wezterm_pane` としてペイロードへ含める（FR-01、§3）。`sessions.wezterm_pane TEXT` 列を追加し `applyMigrations()` の `TABLE_MIGRATIONS`（`sessions` エントリ）へ組み込み、`TerminalDto`/`FocusTarget` に `wezterm_pane`/`weztermPane` を配線（FR-02、§7.3・§7.5・§8.3・§8.4）。`focus-target.ts` に `sanitizeWeztermPane()`（`/^\d+$/` のみ許容）を追加し、新規 `src/cli/focus/wezterm-strategy.ts` の `WeztermFocusStrategy` が `execFile`（非 shell）で `wezterm cli activate-pane --pane-id <id>` を実行（5 秒タイムアウト・in-flight ガード付き、任意の `verifyActivation` で WSL 向け後追い検証）（FR-03、§14.3）。`Strategy.focus()` のシグネチャを `focus(tty)` → `focus(target: FocusTarget)` へ破壊的変更し、darwin 総当たりへ `WeztermFocusStrategy` を追加、WSL2 は `weztermPane` があれば WezTerm を先に試し失敗時 Windows Terminal へフォールバック、ネイティブ Linux を新規サポート（WezTerm 利用時のみ）、tmux 併用時は `weztermPane` を `null` へ縮退（FR-04、§14.1・§14.3）。macOS・ネイティブ Linux は追加設定不要、WSL2 は Windows 側 `.wezterm.lua` への `WSLENV=WEZTERM_PANE` 追記が前提（自動設定はしない設計判断、§14.5）。tmux と WezTerm の併用構成でのペイン特定・iTerm2/VS Code 統合ターミナル対応はスコープ外のまま（§14.1）。                                                                                                                                                                                              |
...
- ターミナルフォーカス（§14）の iTerm2/VS Code 統合ターミナル対応、Linux ネイティブ（X11/Wayland）の WezTerm 以外のターミナル、tmux と WezTerm の併用構成でのペイン特定、ネストした tmux/WezTerm 解決、`wt_session` によるタブ単位フォーカス、`sessions.pid` の充填、`CLAUDE_CODE_DISABLE_TERMINAL_TITLE`/WSL2 `WSLENV` 設定の install-hooks 自動設定（いずれも release-23/release-28-wezterm-focus で strategy 追加等の拡張可能な構造のみ用意）
