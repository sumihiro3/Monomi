...
| Monomi の位置づけ        | テンプレート「導入済みプロジェクト第 1 号」として `workflow.config.json` の `templateVersion`（導入時点のテンプレートリポジトリ HEAD SHA。update スキルの差分検出の起点）・`installedAt` を記録する。以降の更新追従は `/update-release-workflow` の対象になる                                                                                                                                                                  |
...
- AC-3: `.claude/workflow.config.json` の `templateVersion` に導入時点のテンプレートリポジトリ HEAD SHA、`installedAt` に導入日を記録する
