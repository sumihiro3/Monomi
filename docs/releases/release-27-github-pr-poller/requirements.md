...
- AC-4: 受け入れ試験（手動検証必須） — 実機（OSC 8 対応端末、例 Ghostty/iTerm2）で PR 番号をクリックし、
  ブラウザで該当 PR が開くことを確認する。**2026-07-18 実施・確認済み。** 初回検証で「OSC 8 は見た目を
  変えないためクリック可能と分からない」という所見が得られ、追加修正（PR 番号が実際にリンク化される
  場合のみ cyan + underline を付与）を行った上で Ghostty で再検証し、クリックでブラウザに該当 PR
  （このリポジトリ自身の PR #30、隔離した一時 hub 経由）が開くことを確認した。
