...
| テストシナリオ         | AC 項目                    | 結果            | 備考 |
| ---------------------- | -------------------------- | --------------- | ---- |
| 1. hub 自動再起動      | AC-1 hub が新版へ再起動    | ☐ Pass / ☐ Fail |      |
| 1. hub 自動再起動      | AC-2 notice に版情報が表示 | ☐ Pass / ☐ Fail |      |
| 2. reporter 自動再配置 | AC-1 reporter マーカー更新 | ☐ Pass / ☐ Fail |      |
| 2. reporter 自動再配置 | AC-2 notice に版情報が表示 | ☐ Pass / ☐ Fail |      |
| 3. 版一致時スキップ    | AC-4 hub が再起動されない  | ☐ Pass / ☐ Fail |      |
| 5. auto_update 無効時  | AC-6 更新が抑止される      | ☐ Pass / ☐ Fail |      |
| 全シナリオ             | ダッシュボード正常動作     | ☐ Pass / ☐ Fail |      |
...
- [ ] すべてのテストシナリオが Pass
- [ ] hub・reporter が新版に揃っている
- [ ] ダッシュボードが正常に起動・動作する
- [ ] notice の日本語表記（`locale: ja` の場合）が正しい
- [ ] ドキュメント（本チェックリスト）が実装と齟齬がないことを確認
