...
- 対応方針: `filtered()` がフィルタ未選択時に `status.display === 'closed'` の行を除外する（明示的に `5` 種のフィルタで `closed` を選んだ場合のみ表示、という設計であれば `FILTER_ORDER` に closed 用のトグルを追加する等、要件を壁打ちして決める）。関連: ARCHITECTURE.md §6（ライブネス検知未実装。異常終了時は `SessionEnd` 自体が発火せず本問題とは別系統）。
