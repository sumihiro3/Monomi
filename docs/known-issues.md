...
### U1. ヘッダータイトルが「Claude Code Status」のままで、目立たない

- 場所: `src/cli/components/app-view.tsx`（`<Text bold>Claude Code Status</Text>`）
- タイトル文言を「Monomi」に変更し、背景色付与等で視認性を上げたい（ユーザー要望）。Ink の `Text`/`Box` は `backgroundColor` prop を持つため技術的には可能。配色・具体的な強調方法は要壁打ち。

### U2. 「● watching」インジケータを点滅させたい

- 場所: `src/cli/components/app-view.tsx`（`{polling.isRunning() ? <Text color="green">{'  '}● watching</Text> : null}`）
- Ink 標準コンポーネントに点滅（blink）属性は無い。実装するなら `setInterval` 等で表示/非表示を周期的にトグルする自前実装が必要（既知の P4「無条件再レンダー」問題と関連するため、実装時は再レンダー範囲を watching インジケータ周辺に限定する等の配慮が要る）。

### U3. ダッシュボードで選択中カードをもっと目立たせたい（枠線を太くする等）

- 場所: `src/cli/components/instance-card.tsx`（`borderColor={selected ? 'cyan' : undefined}`）
- 現状は borderColor のみで区別。Ink の依存 `cli-boxes` には `borderStyle="bold"`（太字罫線）が存在することを確認済み（`single`/`double`/`round`/`bold`/`singleDouble`/`doubleSingle`/`classic`/`arrow`）。選択中のみ `borderStyle="bold"` に切り替える等で実現可能。

### U4. フィルタリング（`[1]稼働中` 等）とカードグリッドの連動が分かりにくい

- 場所: `src/cli/components/status-filter-bar.tsx`
- 現状、有効なフィルタは `Text inverse={active}` による反転表示のみで、カードグリッド側との視覚的なつながりが弱い（ユーザー報告: 連動している実感が薄い）。対応方針は未検討（例: 非該当カードのグレーアウト、フィルタバーとグリッドを結ぶ視覚要素、件数バッジの強調など）。要壁打ち。
...
### B6. セッション終了（正常終了含む）後も instance が一覧に残り続け、消す手段が無い

- 場所: `src/cli/instance-list-store.ts:68-71`（`filtered()`）、`src/cli/status-display.ts:11`
- `SessionEnd` フックが正常に発火し hub 側で `status.display: 'closed'` を返す状態になっても、`InstanceListStore.filtered()` はフィルタ未選択時（既定表示）に全行をそのまま返すため、closed の instance が一覧に残り続ける。`status-display.ts:11` のコメントは「`closed` は非表示扱いでフィルタ対象に含めない」と意図を明記しているが、実装（`filtered()`）はこれを反映していない。加えて `closed` は `FILTER_ORDER`（`1`-`5`）に含まれない5状態専用のため、ユーザーが明示的にフィルタで隠す手段も無い。ユーザー報告: 「セッションが終わっても（Claude Codeをexitしても）Monomiのプロジェクト一覧には残り続けている」。
- 対応方針（決定済み）: `filtered()` がフィルタ未選択時（既定のファーストビュー）は `status.display === 'closed'` の行を除外する。ユーザーが明示的に見たい場合のみフィルタ経由で表示できるようにする（`FILTER_ORDER` への closed 用トグル追加等、具体的なフィルタ操作は実装時に詰める）。関連: ARCHITECTURE.md §6（ライブネス検知未実装。異常終了時は `SessionEnd` 自体が発火せず本問題とは別系統）。対応リリースは release-7 とは別（release-8 候補）。
