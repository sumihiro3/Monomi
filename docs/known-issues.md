...
### A8. class-diagram.md: InstanceListStore.projectRows() のシグネチャが省略可能引数 rows? を反映していない

- severity: low（review:arch 由来、起票日: 2026-07-13）
- 場所: docs/design/class-diagram.md:600
- 現象: class-diagram.md 600行目は `+projectRows() ProjectRow[]` と記載しているが、src/cli/instance-list-store.ts の実装は `projectRows(rows?: InstanceStatusRow[]): ProjectRow[]` に変更された（FR-03 AC-1: 呼び出し側が既に計算済みの filtered 結果を渡すことで二重計算を回避）。図と実装のシグネチャ齟齬。796行目の責務説明「`projectRows()`（`ClientRollup` 委譲）を提供する」もオプショナル引数について触れていない。
- 対応方針: class-diagram.md 600行目のシグネチャ記載を `projectRows(rows?: InstanceStatusRow[]): ProjectRow[]` へ更新し、796行目の責務説明にもオプショナル引数の意図（呼び出し側が既に計算済みの filtered 結果を渡すことで二重計算を回避する）を追記する。
...
### A9. 段階リリースと現状スコープ table に release-20 エントリが欠落

- severity: low（review:arch 由来、起票日: 2026-07-13）
- 場所: docs/ARCHITECTURE.md:499-518
- 現象: ARCHITECTURE.md の本文（§10.3 の PollingLoop stop/start 記述）およびクラス図注記（MemoryWatchdog / KeyBindingController の release-20-dashboard-heap-guard 参照）には release-20 への言及が複数あるが、末尾の「段階リリースと現状スコープ」サマリーテーブル（lines 499-518）は release-19 で止まっており release-20 の行が無い。ARCHITECTURE.md は自らを「権威仕様」と宣言しており、同一ドキュメント内で release-20 の設計仕様が本文に散在する一方でサマリーに欠落する不整合が生じている。
- 対応方針: テーブルに release-20 行（FR-01 MemoryWatchdog / FR-02 isStdoutBackpressured / FR-03 handleKey boolean / FR-04 PollingLoop stop/start）を追加すべき。
