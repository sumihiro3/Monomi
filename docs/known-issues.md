...
### B9. 孤立セッション（`SessionEnd`/`Stop` を一度も送らず異常終了した過去セッション）が rollup の「live」候補として無期限に残り、直近セッションが正常終了した直後にそれを乗っ取って「終了」ではなく「放置」表示にすり替わる

- 場所: `src/status/instance-status-rollup.ts`（`InstanceStatusRollup.rollup()` の `live` フィルタが `entry.status.display !== 'CLOSED'` のみで候補判定しており期限がない）、`src/status/escalation.ts`（`EscalationPolicy.classify()`、`CLOSED` 以外は無条件に放置閾値判定の対象）
- 現象: 2026-07-09、Monomi 自身のリポジトリ（instance `inst_24529f7c-8e85-47f7-9763-8ac87e37f900`、`/opt/dev/Monomi`）で実機の `monomi.db` を直接確認して再現を確認。session `6cebbfd8`（2026-07-03 19:26 開始）はターミナルを閉じる等の異常終了で `SessionEnd`/`Stop` 相当の終了イベントを一度も送っておらず、`sessions.ended_at` が未設定のまま（`raw_state` が `CLOSED` に遷移せず恒久的に `live`）。その後 session `b1008e01`（2026-07-07 11:18〜2026-07-09 18:23、`/exit` 等で正常終了し `SessionEnd` を実際に受信済み）が同じ instance 上で2日以上稼働していた間は、`InstanceStatusRollup` の recency 優先（release-8 FR-02、B8 対応）により `b1008e01` が代表となり、孤立セッション `6cebbfd8` は覆い隠されていた。しかし `b1008e01` が `SessionEnd` を送って正常終了（`CLOSED`）した瞬間、rollup の `live` フィルタから `b1008e01` 自身が除外され、instance 内で唯一残る「live」候補が6日前に放棄された孤立セッション `6cebbfd8` になる。`6cebbfd8` の最終イベントは 2026-07-03 23:09 で `next_wait` の放置閾値（既定24h）を大幅に超過しているため即座に `STALE`（放置）と判定され、経過時間は約5〜6日となる。結果、ユーザーが直前に正しく `/exit` したセッションの「終了」表示は一切現れず、6日前に放置されたまま忘れられていた無関係な孤立セッションの「放置 5d」表示だけが instance 全体の代表として浮上する（ユーザー報告: 「claude code セッションをexitしたはずなのに『放置』となっている」）。
- 背景: release-8（B8 対応、FR-02）で孤立セッション除外の閾値ロジック（release-7 `STALE_SESSION_THRESHOLD_MS`、15分）を撤廃し「`CLOSED` 以外はすべて live」という単純な recency 優先ルールへ全面移行した。「新しい session が古い状態に覆い隠される」問題（B8）の解決としては正しかったが、副作用として「一度も `CLOSED` になれない孤立セッション」が期限なく live 候補であり続けるようになった。真に稼働中のセッションが存在する間は recency により隠れるため顕在化せず、そのセッションが正常終了した瞬間にだけ表面化するため発見しにくいタイミング依存の回帰である。`docs/ARCHITECTURE.md:175` は「孤立 session 自体を `CLOSED` へ確定させる根本対応（ライブネス検知）ではない」と一般論として既に明記済みだが、「正常終了直後に無関係な孤立セッションの放置状態が代表に浮上する」という具体的な症状としては known-issues.md に未起票だった。
- 対応方針: (a) rollup の live 判定に「対応する raw_state の放置閾値を超えて更新のない孤立セッション（`ended_at IS NULL` かつ長期未更新）は、他に live 候補が無い場合に限り代表から除外し、instance 全体を『終了』相当（直近に `CLOSED` した session を代表にフォールバック、全 session が該当条件なら非表示）として扱う」という二段目のフィルタを追加する、(b) または一定期間（例: 対応閾値の数倍）更新のない孤立セッションを検出時に `end_reason='orphaned'` 等を付与し明示的に `CLOSED` 相当へ倒すバッチ/経路を設ける、のいずれか（または組み合わせ）を検討する。根本解決には `docs/ARCHITECTURE.md` §6 が未実装として明記するライブネス検知（heartbeat/`session_lost`）の実装が必要。

### B10. LANG_DELIMITER_RE に `@` 区切り文字が欠落しており、`LANG=ja@modifier` 形式の値を検出できない

- severity: low（review:bugs 由来、起票日: 2026-07-09）
- 場所: src/i18n/os-locale.ts:8
- 現象: 要件 FR-02 AC-1 は「`_`/`.`/`@` より前の言語サブタグを抽出」と明記しているが、正規表現 `LANG_DELIMITER_RE = /[_.]/` に `@` が含まれていない。POSIX ロケールの書式は `language[_territory][.codeset][@modifier]` であり、地域・エンコーディングを省略して修飾子だけを付けた `ja@calendar=japanese` のような値が `LANG` に設定された場合、`split(/[_.]/)[0]` の結果は `'ja@calendar=japanese'` となり `'ja'` に一致しないため `undefined` を返す（期待値は `'ja'`）。テスト側にもこのケースのカバレッジがない。実運用では `ja_JP.UTF-8` 形式が大半のため低頻度だが、仕様に対して不正確。
- 対応方針: 正規表現を `LANG_DELIMITER_RE = /[_.@]/` に修正すれば AC-1 の仕様どおりに動作する。あわせて `ja@modifier` 形式のテストケースを追加する。
- 追記(2026-07-09, severity: low, review:arch 由来): 別のレビューでも同事象を検出。要件 FR-02 AC-1 が指定する `@` デリミタが `LANG_DELIMITER_RE = /[_.]/` に含まれておらず、`LANG=ja@modifier` のようなケースで `ja@modifier` が抽出され `ja` にマッチせず `undefined` が返る。実運用での日本語・英語ロケールで `@` 修飾子が使われることは極めて稀だが、要件定義との不一致であり、修正は `LANG_DELIMITER_RE = /[_.@]/` への変更のみで済む。
