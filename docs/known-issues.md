...
### U8. バックグラウンド Workflow 稼働中に running-work 表示（`▶ <name>`）が消灯し、別の Skill 名にも化ける

- 場所: `src/status/running-work-resolver.ts`（消灯境界と候補選定）。関連: `src/hub/instance-status-service.ts`（ACTIVE ゲート・駆動側）
- 現象: バックグラウンド起動の Workflow はツール呼び出し自体が数秒で完結（`PreToolUse`→`PostToolUse` が即ペア化）し、直後のターン終了で `Stop` が発火する。resolver は `Stop`/`UserPromptSubmit`/`Notification(idle_prompt)`/`SessionEnd` を消灯境界とし、境界より新しい `PreToolUse` しか候補にしないため、`PreToolUse|Workflow` イベントが境界の向こう側に取り残される。一方 Workflow 本体は subagent のフックイベント（Read/Bash 等）で raw_state=ACTIVE を維持したまま長時間稼働するので、「ACTIVE なのに running_work=null（表示 `-`）」になる。稼働中に別の Skill/Agent を呼ぶと今度はその名前が表示され、Workflow 名には戻らない。
- 実例: 2026-07-07、release-17 の run-release（実行 30〜50 分）稼働中に表示が出ないことをユーザーが確認。live API で raw_state=active かつ `running_work={"kind":"skill","name":"investigate"}`（直近に呼んだ別 Skill）を実測。hub DB には `PreToolUse|Workflow|run-release` が正しく記録されており reporter は正常（release-16 の名前抽出は scriptPath 起動でも機能している）。
- 背景: release-16 実装時、「バックグラウンド実行のため PostToolUse が即発火する」問題は認識され PostToolUse を候補から除外済み（`runningWorkCandidateOf` のコメント）だが、同じ理由で 1 ステップ後に来る `Stop` 境界が同効果を持つ点が未対処（「Workflow の寿命 ≠ ターンの寿命」の境界側）。
- 対応方針: 消灯境界の再設計が必要、要壁打ち。候補: (a) ACTIVE 継続中は境界を跨いで最新の Workflow 候補まで遡る、(b) Workflow 完了を示すシグナルの導入（現行フックに完了通知イベントは無い）。
...
### U10. `npx monomi-cli` 一発で hub + ダッシュボードが立ち上がるクイックスタートにしたい

- 場所: 該当なし（未実装機能）。関連: `src/cli.ts`（起動フロー）、`src/hub/serve.ts`
- 内容: `npx monomi-cli` 一発で初回セットアップ（install-hooks）→ hub 自動起動 → ダッシュボード表示まで完結させ、そのマシンをそのまま hub にする。以降のマルチデバイス化は「別マシンから `monomi pair` するだけ」にしたい（ユーザー要望、2026-07-07。claude-code-monitor の 1 コマンド導入が比較対象）。
- 設計方向（壁打ち済みの骨子）: 起動時に既定ポートの疎通を確認し、hub 不在なら `process.execPath` + パッケージ内 cli.js で `monomi hub` を detached spawn する（自己デーモン化。npx キャッシュ実行でも `import.meta.url` で自パス解決可能）。「いつでも別マシンから pair できる」ためには hub はダッシュボード終了後も生存し、現行既定の `0.0.0.0` + token 認証で待ち受ける。hub 停止中のイベントは reporter の outbox 退避（復帰後再送）がそのまま補完する。データ経路は現行の 1 本（書き込み主体は常に hub）を維持し、CLI が DB を直読みする hubless 案は採らない（導出経路の二重化と移行問題を避ける）。
- 実装スコープの本体: ライフサイクル管理一式 — `monomi hub stop`/`status` サブコマンド、pid ファイル、二重起動検知（EADDRINUSE を「既に稼働中」として素通し）、クラッシュ後の stale pid 処理。
- 対応方針: release-18 候補。詳細は要壁打ち。**npm への初公開（release-17 FR-05 AC-2 の実 publish）は本項の実装後に行う方針**（2026-07-07 ユーザー決定。導入体験が npx 一発になってから世に出すため。それまで v* tag push は行わない）。
