...
> **本資料は凍結済み。現行仕様は `ARCHITECTURE.md` を参照すること。**
> 本資料は設計検討の経緯（命名決定・確定仕様修正の背景等）の記録として凍結されており、以後 `sync-docs` の同期対象外とする。`ARCHITECTURE.md` へ移行済みの節には、節見出し直下に移行先ポインタを付す。
...
> → 現行は `ARCHITECTURE.md` §1「確定仕様の要点（handoff §0）」参照。
...
> → 現行は `ARCHITECTURE.md` §2「全体アーキテクチャ」（hub/child 役割分担・クロスプラットフォーム方針・常時稼働を含む）参照。
...
> → 現行は `ARCHITECTURE.md` §4「フック → raw_state マッピング」参照。
...
> → 現行は `ARCHITECTURE.md` §5「status 導出ロジック」参照。
...
> → 現行は `ARCHITECTURE.md` §6「異常終了・ライブネス（現状: 未実装）」参照。本節のハートビート方式は設計のみで、v1 では未実装（放置しきい値のセーフティネットに委ねている）。
...
> → 現行は `ARCHITECTURE.md` §7「データモデル」参照。DDL は実装（`src/`）と齟齬がないか随時 `sync-docs` で確認される。
...
> → 現行は `ARCHITECTURE.md` §8「Hub API 設計」参照。
...
> → 現行は `ARCHITECTURE.md` §9「ペアリングフロー」参照。
...
> → 現行は `ARCHITECTURE.md` §10「CLI 設計」参照。
