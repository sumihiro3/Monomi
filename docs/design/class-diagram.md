...
classDiagram
  ...
  InstanceStatusRollup ..> RollupEntry : uses (stale exclusion)
...
- `InstanceStatusRollup`: 1 instance 配下の複数 `RollupEntry`（`StatusResult` ＋ 該当 session の直近イベント時刻 `lastEventAt`）から代表 `StatusResult` を選ぶ。選定前に、instance 内で最も新しい `lastEventAt` から 15 分（`STALE_SESSION_THRESHOLD_MS`、モジュール内定数）以上離れた session を「孤立 session」として候補から除外し、残った候補だけで `StatusPriority.higherOf` の畳み込みを行う（release-7 FR-01、`known-issues.md` B7 対応）。基準は hub の絶対時刻ではなく instance 内の相対距離とし、長時間ツール実行中で新規イベントが無い稼働中 session 自身が誤って stale 化しないようにする。`RollupEntry` の組み立て（`lastEventAt` に `loadEventsForCurrentRun` 先頭の `receivedAt` を採用）は呼び出し元の `InstanceStatusService.buildRow` が担う。
...
| InstanceStatusRollup                                                        | status-engine | domain service                                  | instance 配下の session から代表ステータスを選出（孤立 session を15分閾値で除外、release-7 FR-01） |
