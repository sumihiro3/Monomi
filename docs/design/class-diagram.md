...
classDiagram
  ...
  class SessionTerminal {
    <<interface (value object), release-23 FR-02b. release-28 FR-02: weztermPane 追加>>
    +string tty
    +string termProgram
    +string tmuxPane
    +string tmuxSocket
  ...
  class PrStatusMapper {
    <<module (pr-status-mapper.ts), release-27 FR-02>>
    +mapPrToStatus(pr: GhPrInfo|null) MappedPrStatus$
  }
  class EpochMs {
...
classDiagram
  ...
  class GithubPrPoller {
    <<application service (github-pr-poller.ts), release-27 FR-01/FR-01a/FR-01b. 既知課題 U7 解消>>
    +start() Promise
    +stop() void
    +isRunning() bool
    +pollOnce() Promise
    -collectTargets() Map
    -pollBranch(target: PollTarget, signal: AbortSignal) Promise
  }
  class TokenService {
    <<domain service>>
  ...
  class HubLifecycle {
    <<module (hub-lifecycle.ts), release-18 FR-02. release-25-auto-update FR-01/FR-02: probeHub で版取得を追加>>
    ...
    +probeHub(port: int, options: ProbeHubOptions) Promise$
  ...
  GithubPrPoller ..> InstanceRepository : listActive (collectTargets)
  GithubPrPoller ..> ProjectRepository : findById (collectTargets)
  GithubPrPoller ..> PrStatusRepository : upsert
  GithubPrPoller ..> PrStatusMapper : mapPrToStatus (release-27 FR-02)
...
- **配線** は `createHubServer`（factory）が担い、Repository → Service → Controller → `Router` を組んで `HttpServer` を返す。閾値（`EscalationThresholds`）と権威時刻（`now`）、hub 自身の版文字列（`hubVersion`、既定 `MONOMI_VERSION`、release-25-auto-update FR-01）はここで注入する。
...
- **hub 版のレスポンスヘッダ公開（`HttpServer.send()`、release-25-auto-update FR-01）**: 全応答（404/405/401/413/400/500/200 すべて）が通る唯一の送出チョークポイントである private `send()` が `X-Monomi-Hub-Version` ヘッダを常時付与する。未認証の 401 応答にも版を開示する意図的設計（既知課題 S15 として記録）。`HubLifecycle.probeHub`（本 §、到達性プローブ側）・cli-ink 側 `HubApiClient.getLastHubVersion()`（§4）がこのヘッダを読み取り、FR-02（hub 自動再起動）・FR-04（child のリモート hub 版ずれ可視化）の版比較（`VersionCompare.compareVersion`、§4）の入力になる。
...
- **`GithubPrPoller`**（`src/hub/github-pr-poller.ts`、release-27 FR-01/FR-01a/FR-01b、既知課題 U7 解消）: `InstanceRepository.listActive()` から GitHub リモート（`projectKey.kind === 'GIT_REMOTE'` かつ `github.com/` 接頭辞）× branch の組を重複排除して収集し（`collectTargets`）、branch ごとに `gh pr list --repo <owner>/<repo> --head <branch> --state all --json number,state,reviewDecision,isDraft,url` を `execFile`（非 shell、`ExecFileFn` DI）で呼ぶ。取得結果は `PrStatusMapper.mapPrToStatus`（§1）で `pr_status.state`/`isDraft` へ写し、`PrStatusRepository.upsert()` へ永続化する（従来 `InstanceStatusService` の `hasPrWaiting` 導出が定数 `false` 固定だった配線バグの実データ側を埋める）。`gh` 未導入・未認証は起動時 `gh auth status` の成否で 1 回だけ判定して以後 no-op に縮退し（AC-4）、稼働中の認証失効も全 branch 失敗時に再確認して同じ縮退経路へ倒す。1 サイクルは `AbortController` で直列化（前サイクル未完了なら新規サイクルを起動しない）し、`stop()` は timer 停止に加え進行中の `gh` 子プロセスも abort する。`hub/serve.ts` の `serve()` が config `github_pr_poll`（`enabled`/`interval`/`allowed_repos`）から組み立てて起動し（`HubHandle.githubPrPoller` として公開）、`close()` は `server.close()`/`db.close()` より先に `stop()` を呼びタイマーがプロセス終了をブロックしないようにする（`serve.ts`/`HubHandle` 自体は本図対象外の起動エントリ、`HubLifecycle` 呼び出しと同様の扱い）。
- **`SessionRepository.updateTerminal`（release-23 FR-02b、release-28 FR-02 で `wezterm_pane` 追加）**: reporter が捕捉したターミナル特定情報（`tty`/`term_program`/`tmux_pane`/`tmux_socket`/`wsl_distro`/`wt_session`/`wezterm_pane`）の最新スナップショットで `sessions` の該当行を無条件上書きする。呼び出し元 `EventIngestionService.ingest()` は `payload.terminal` が undefined/null でないときにのみこれを呼ぶ規約（旧 reporter の欠落ペイロードで既存スナップショットを NULL 上書きしないため、FR-02 AC-5）。`InstanceStatusService.buildRow()` はこの値を `toTerminalDto()`（`dto.ts`）経由で `SessionDto.terminal` へ写し、cli-ink 層の `FocusService`（§4）が `HubApiClient` 経由で読む TerminalDto の発生源になる（cli→hub の依存方向は変わらず、hub 側は `FocusService`/`focus/` を一切 import しない）。
...
- **hub ライフサイクル管理**（`HubLifecycle`、`src/hub/hub-lifecycle.ts`、release-18 FR-02、既知課題 A7 解消）: `~/.monomi/hub.pid` の書込・削除（`writeHubPidFile`/`removeHubPidFile`）と、pid 生存確認（`isProcessAlive`）＋ port 疎通確認（`isPortReachable`）を突き合わせた3状態判定（`hubStatus`: `running`/`stopped`/`stale`）・生存確認済み pid への SIGTERM 送信＋終了ポーリング（`hubStop`）を1モジュールに集約する。呼び出し元は2方向: (1) `hub/serve.ts` の `serve()`/`close()` が起動成功時・正常終了時に `writeHubPidFile`/`removeHubPidFile` を直接呼ぶ（同一 hub レイヤー内の依存）。(2) `monomi hub status`/`monomi hub stop`（`cli.ts`）が `hubStatus`/`hubStop` を呼ぶ（cli.ts は本図の対象外の起動エントリ、既存の `serve.ts`/`bootstrap.ts` と同様に扱う）。`isPortReachable` は cli-ink 層の `HubEndpointResolver.isReachable`/`HubEndpointOps.localhostEndpoint`（§4）と同じ「GET して応答があれば（ステータス不問で）到達」という判定パターンだが、hub レイヤーが cli レイヤーへ依存しない方針（hub→status の依存方向を保つのと同じ理由）のためこのモジュール内に意図的な別実装として持つ（ロジックの二重実装より依存方向の一貫性を優先）。この `isPortReachable` は cli-ink 層 `HubAutostart`（§4）が唯一 import するクロスレイヤー依存（cli→hub の一方向、hub→cli は無い）。**release-25-auto-update FR-01/FR-02**: 到達性のみだった `isPortReachable` を、応答ヘッダ `X-Monomi-Hub-Version`（`HttpServer.send()` が付与）から hub の版まで同時に読み取る `probeHub` へ発展させた。`isPortReachable` は `probeHub` の到達性のみを返す薄いラッパーとして後方互換を維持する（既存呼び出し元 `HubAutostart.waitUntilReachable` はこちらを使い続ける）。`hubStatus` は `probeHub` の結果を `HubStatusResult.hubVersion` へ充填し（ヘッダ欠落＝旧版 hub なら `undefined`）、`monomi hub status` の表示（`cli.ts` の `formatHubStatus`、本図対象外）が「稼働中です（pid ..., port ..., version ...）」の版表示に使う。`HubAutostart.ensureHubRunning`（§4 FR-02）は `probeHub` で得た版を `VersionCompare.compareVersion`（§4）に渡し、hub 自動再起動の要否を判定する。
...
classDiagram
  ...
  class HubAutostart {
    <<module (hub-autostart.ts), release-18 FR-01. release-25-auto-update FR-02: 版照合〜graceful 再起動へ拡張>>
  ...
  class VersionCompare {
    <<module (version-compare.ts), release-25-auto-update>>
    +compareVersion(other: string|undefined, self: string) VersionComparison$
  }
  class VersionComparison {
    <<type alias>>
    older_same_newer_unknown
  }
  ...
  class Osc8Hyperlink {
    <<module (osc8-hyperlink.ts), release-27 FR-05b>>
    +isLinkableGithubUrl(url: string|null) bool$
    +toOsc8Hyperlink(text: string, url: string|null) string$
  }
  ...
  class FocusTarget {
    <<interface (value object), release-23 FR-04. release-28-wezterm-focus FR-03a: weztermPane 追加>>
    +string|null tty
  ...
  class Strategy {
    <<interface (focus/types.ts), release-28-wezterm-focus FR-04-pre: focus(tty)→focus(target) 破壊的変更>>
  ...
  class WeztermFocusStrategy {
    <<strategy (focus/wezterm-strategy.ts), release-28-wezterm-focus FR-03b/FR-04>>
    +matchesHint(target: FocusTarget) bool
    +focus(target: FocusTarget) Promise
    -runFocus(paneId: string) Promise
    -verifyPaneExists(paneId: string) Promise
  }
  class FocusService {
    <<application service (focus/focus-service.ts), release-23 FR-04. release-28-wezterm-focus FR-04: 3→4系統ディスパッチへ再設計>>
    +focus(target: FocusTarget) Promise
  }
  class Osascript {
  ...
  note for ExecFileFn "`osascript.ts`・`tmux-strategy.ts`・`wsl-strategy.ts`・`wezterm-strategy.ts`（release-28-wezterm-focus FR-03b）がそれぞれ独立に同形の `ExecFileFn`（+ 既定実装）を宣言する（共有モジュール化はしていない）。`wezterm-strategy.ts` の既定実装のみ `options.timeout`（既定5秒）を渡す点が他と異なる。図では1ノードにまとめて表現。"
  ...
  DetailView ..> Osc8Hyperlink : toOsc8Hyperlink (pr 番号のリンク化, release-27 FR-05b)
  ...
  FocusService ..> Strategy : dispatch darwinStrategies (term_program hint order, WeztermFocusStrategyも含む)
  FocusService ..> TmuxStrategy : tmuxPane あれば switchClient() を最優先で呼ぶ (成功時 weztermPane は null へ縮退, release-28 FR-04 AC-5)
  FocusService ..> Strategy : weztermWslStrategy (WSL2, weztermPane優先→wslStrategy fallback) / weztermStrategy (ネイティブLinux, release-28 FR-04)
  FocusService ..> WslStrategy : WSL2 で weztermPane 無し/weztermWslStrategy 未配線/失敗時に focus() を呼ぶ
  TmuxStrategy ..> TmuxSwitchOutcome : returns
  TerminalAppStrategy ..|> Strategy : implements
  GhosttyStrategy ..|> Strategy : implements
  WeztermFocusStrategy ..|> Strategy : implements
  ...
  WeztermFocusStrategy ..> ExecFileFn : own execFile DI (not via Osascript, command差替で darwin/nativeLinux='wezterm' / WSL2='wezterm.exe')
  ...
  DetailView ..> I18n : t() (pr.* ラベル/draft 注記, release-27 FR-05c/FR-05d)
  ...
  HubAutostart ..> VersionCompare : compareVersion (hub 版照合, release-25-auto-update FR-02)
  AppView ..> VersionCompare : compareVersion (remoteHubNotice, release-25-auto-update FR-04)
  VersionCompare ..> VersionComparison : produces
...
- `HubApiClient` は `baseUrl` 非依存の読み取り/管理クライアント（一覧・詳細・devices 一覧/revoke・pair start/claim）。到達先の選定は持たず、`createHubApiClient`/`createHubConnection`（factory）が `HubEndpointResolver` で候補を優先順にプローブして配線する。`createHubConnection` は再解決ファクトリ同梱の `HubConnection` を返し、`PollingLoop` の watch 中フォールバックに供給する。直近の Bearer 付き GET 応答（`listInstances`/`getInstanceDetail`/`listDevices`）から読み取った `X-Monomi-Hub-Version` ヘッダを `getLastHubVersion()`（release-25-auto-update FR-04）で公開し、追加リクエスト無しで接続中 hub の版を監視できるようにする（`AppView` が `VersionCompare.compareVersion` と組み合わせて `remoteHubNotice` を導出。上記参照）。
...
- `HubAutostart`（`ensureHubRunning`、release-18 FR-01、既知課題 A7 解消）は `monomi`（引数なし）実行時にダッシュボード表示前で呼ばれる自己修復自動起動の実体。`role: child` では何もせず（`role` が `child` 以外なら hub 起動対象と見なす）、既に `port` へ疎通できれば何もしない。疎通できなければ自パッケージ内 `dist/bin.js` を `process.execPath` で `hub` サブコマンド付き detached spawn（`unref()` 済み、`spawnHub` private helper）し、stdout/stderr を `~/.monomi/hub.log` へ追記リダイレクトしてリトライ付きで起動完了を待つ（既定 10 秒でタイムアウトし `hub.log` 参照を含む例外を投げる）。疎通確認そのものは hub-api 層 `HubLifecycle.isPortReachable`（§3 参照）を直接 import して使う——これが本図中で唯一の cli-ink→hub-api のクロスレイヤー依存（逆方向の hub→cli 依存は無い）。タイムアウト時のエラーメッセージ整形は他の cli-ink モジュールと同様 `I18n.t()` 経由（`hubLogFile` 参照を含む）。Ink コンポーネント・`HubApiClient` には依存しない（ダッシュボード起動より前段で完結する処理のため）。
  - **release-25-auto-update FR-02（hub 版照合〜自動再起動）**: 既に疎通できる場合、`HubLifecycle.probeHub`（§3。応答ヘッダから hub 版も同時取得）で得た hub 版と自版（`MONOMI_VERSION`）を `VersionCompare.compareVersion` で比較する。`same` なら何もしない。`newer`（hub の方が新しい）は hub に触れず「CLI が旧版」notice のみ返す（新旧 CLI 交互実行によるフリップフロップ防止）。`older`/`unknown`（版不明は旧版扱い）は `config.yml` の `auto_update`（既定 `true`）が `false` なら版ずれ notice のみ、`true` なら `HubLifecycle.hubStop` で graceful 停止 → 成功時のみ `spawnHub` で新版を再 spawn する（停止に失敗すれば SIGKILL へはエスカレーションせず「更新失敗・旧版継続」notice を返す）。戻り値は `void` から `Promise<string | null>`（i18n 解決済み notice 文字列、無ければ `null`）へ変更された破壊的変更で、呼び出し元 `cli.ts` がこれを `startupNotices` へ集約する。
- **起動時 notice チャネル（`AppView` の `startupNotices`/`remoteHubNotice`、release-25-auto-update FR-02〜FR-04）**: `cli.ts` は `HubAutostart.ensureHubRunning`・reporter 版照合の `ensureReporterUpToDate`（`install-hooks.ts`、本図の対象外モジュール。FR-03）が返す notice（`null` 以外）を集約し、`AppView` の `startupNotices: string[]` prop としてヘッダー直下に警告色（黄）でスタック表示する永続領域へ橋渡しする（自動消去しない。`f` フォーカス失敗時の約4秒で消える時限式 `notice` state とは独立）。`AppView` はこれとは別に、watch ポーリングの取得関数内で `HubApiClient.getLastHubVersion()`（追加リクエスト無し）を読み `VersionCompare.compareVersion` で自版と比較し、接続中のリモート hub が `older`/`unknown`（版不明も旧版扱い）なら同じ永続領域に `remoteHubNotice` を表示する（FR-04）。`same`/`newer` に戻れば notice を消し、直前と同一文字列なら再セットしない dedup により表示は常に高々1件（連続ポーリングでの増殖防止）。
...
- `Osc8Hyperlink`（`osc8-hyperlink.ts`, release-27 FR-05b）も同じく React 非依存の純粋関数モジュール。`terminal-title.ts`／`ghostty-strategy.ts` と同じ組み立て方（生の制御文字を直接埋め込まず `String.fromCharCode` で名前付き定数化）で OSC 8 ハイパーリンクエスケープを生成し、`DetailView` の `pr` フィールドで PR 番号（`#<number>`）をクリッカブル化するために使う。`toOsc8Hyperlink(text, url)` は `isLinkableGithubUrl(url)`（`URL` でのスキーム/ホスト/資格情報なし/`/owner/repo/pull/<number>` パス形式/制御文字なしの厳密検証）を通らない場合はエスケープを一切生成せず `text` をそのまま返す（プレーンテキストへのフォールバック、hub 側 `gh` CLI 応答由来の未検証 URL を「認証済みだが信頼しない」既存の脅威モデルで扱う）。
...
- **`src/cli/focus/` — フォーカス実行モジュール群（release-23 FR-04、既知課題 U9 解消。release-28-wezterm-focus FR-03〜FR-04 で WezTerm 対応を追加。ネイティブ Linux 対応は当初スコープに含めていたが実機検証を受けて撤回、既知課題 U21。詳細は `docs/ARCHITECTURE.md` §14）**: `AppView.focusTerminal()`（`f` キー、`KeyBindingHost` 実装。§10.3）が呼ぶ `FocusService.focus(target)` を頂点に、対象 TTY／WezTerm ペインへ実際にフォーカスを移す一連のモジュール。`FocusTargetOps.toFocusTarget()` が hub API の `TerminalDto`（wire、`SessionDto.terminal`。実体は `hub/dto.ts` の型を re-export し、CLI 側に独自定義は持たない）を厳格検証して `FocusTarget`（camelCase、CLI 内部用）へ写し、不合格フィールドは個別に `null`（情報なし）へ縮退させる（オブジェクト全体は拒否しない）。release-28 FR-03a: `weztermPane` は `/^\d+$/`（数字のみ）で検証する `sanitizeWeztermPane()` を追加。`AppView` はこの変換結果と `status.display`/`device.id` のゲート判定（§14.2）を経てから `FocusService` を呼ぶ——ゲート判定自体は `FocusService` の責務ではなく host（`AppView`）側に置く設計（`KeyBindingController` が判断を持たないのと同じ「判定は呼び出し側、実行は下位モジュール」の分離）。
  - `FocusService.focus()` のディスパッチは 4 系統に分かれる（release-28-wezterm-focus FR-04 で 3→4 系統へ拡張、`Strategy.focus()` の引数は `tty: string` から `target: FocusTarget` へ破壊的変更——`WeztermFocusStrategy` が `tty` ではなく `target.weztermPane` を必要とするため既存 4 strategy 全てがこれに追従）。**tmux**: `tmuxPane` があれば他のどの strategy よりも先に `TmuxStrategy`（実装は `TmuxFocusStrategy`）の `switchClient()` を呼び、戻り値 `TmuxSwitchOutcome`（`{ result: 'ok', tty }` か `{ result: 'tmux_detached' | 'error' }` の判別可能ユニオン）で解決した外側クライアントの TTY に差し替えて以降の判定を続ける（tmux ペイン内の pts では外側ターミナルを特定できないため）。あわせて `target.weztermPane` を `null` へ縮退させる（tmux 切替前に捕捉した pane id が切替後の外側クライアントに対応するとは限らず、誤ったペインを前面化しうるため。tmux+WezTerm 併用はスコープ外）。以降の判定は tty ベースのフォールバックのみを使う。tty と weztermPane の両方が無ければ `no_terminal`（tty 単独では強制終了させない。`weztermPane` のみでも WezTerm 経路が機能しうるため）。**darwin**: `platform === 'darwin'` なら `Strategy`（`matchesHint`/`focus(target)` の共通インターフェイス）実装の `TerminalAppStrategy`／`GhosttyStrategy`／`WeztermFocusStrategy`（`command: 'wezterm'`）を `term_program` ヒント順に総当たり（iTerm2 等の追加はこの配列へ strategy を足すだけで済む、拡張点）。**WSL2**: darwin でなく `isWsl()`（`WSL_DISTRO_NAME` または `/proc/version` の `microsoft`）が true なら、`target.weztermPane` があり `weztermWslStrategy`（`WeztermFocusStrategy`、`command: 'wezterm.exe'`、`verifyActivation: true`）が配線済みならこれを先に試し、`ok` なら確定。それ以外（`weztermPane` 無し／未配線／失敗）は従来どおり `WslStrategy`（実装は `WslFocusStrategy`、`matchesHint` を持たず darwin 総当たり対象にも含めない）の `focus()` へフォールバックする。**ネイティブ Linux**（darwin でも WSL でもない）: ディスパッチ構造としては `target.weztermPane` があり `weztermStrategy`（`Strategy`）が配線済みならその結果を返す形を持つが、`cli.ts` の既定実装はこれを意図的に配線していない（release-28-wezterm-focus 実機検証で macOS の `activate-pane` 単体では前面化しないと判明し、X11/Wayland 非依存な前面化手段を検証できなかったため。既知課題 U21）。よって実際には常に `unsupported_platform` を返す（release-28 以前と同じ挙動）。
  - `TerminalAppStrategy`／`GhosttyStrategy` は `Osascript.runOsascript()`（`execFile` 非 shell 起動、`ExecFileFn` 型で DI 可能。`HubAutostart`（§3）の `SpawnFn` と同じ「実行時は `node:child_process` を注入・テストは mock」パターンを踏襲）と `AppleScriptOps.escapeAppleScriptString()`（AppleScript へ埋め込む文字列の必須エスケープ）を介して `osascript` を起動する。release-28-wezterm-focus 所見対応: 両 strategy とも `focus(target)` の引数が `tty` 単独から `target: FocusTarget` へ変わったことで「呼び出し側が非 null tty を保証する」前提が崩れたため、自身で `target.tty === null` を検証し `not_found` を返すよう改めた（`focus-service.ts` が tty 単独では総当たりを止めなくなったため）。`TmuxFocusStrategy`／`WslFocusStrategy`／`WeztermFocusStrategy` は AppleScript を使わないため `Osascript` に依存せず、`tmux`／`powershell.exe`／`wezterm`（または `wezterm.exe`）をそれぞれ独立宣言の `ExecFileFn`（同形だが共有モジュール化はしていない）経由で直接 `execFile` する。reporter 由来の値（`tty`/`tmux_pane`/`tmux_socket`/`wezterm_pane`）は「認証済みだが信頼しない」（既知課題 S9/S12 と同じ脅威モデル）ため、いずれの経路も `FocusTargetOps` の厳格検証・（AppleScript を使う場合のみ）`AppleScriptOps` のエスケープ・`execFile` 非 shell 起動という防御を経て初めて外部プロセスへ渡る。release-24 FR-06（既知課題 B12 解消）: `TerminalAppStrategy`／`GhosttyStrategy` はいずれも対象アプリを実際に操作する前に System Events の `exists process` で起動確認するガードを追加し、未起動なら `not_found` を返して以降の操作（`tell application`/TTY への OSC タグ書き込み）自体を行わない（対象外アプリの意図しない自動起動・タブタイトルの一瞬変化を防止）。`GhosttyStrategy` の `finally` タグ消去は、タグ書き込みに成功した試行が 1 回でもあったときのみ行う（一度も書き込んでいなければ消去自体を省略する）。
  - **`WeztermFocusStrategy`（`focus/wezterm-strategy.ts`、release-28-wezterm-focus FR-03b/FR-04、新規）**: `target.weztermPane` が指すペインへ `<command> cli activate-pane --pane-id <paneId>` を `execFile` 非 shell で実行して前面化する。コンストラクタ引数 `command`（単一文字列、または `ENOENT` 時に次候補へ進む配列）の違いだけで darwin・WSL2 interop の2経路に使い回す（`cli.ts` が用途ごとに別インスタンス配線。ネイティブ Linux 向けの3経路目は既知課題 U21 により未配線）。`matchesHint` は `term_program === 'WezTerm'` で darwin 総当たり順を決める。exit 0 → `ok`、`ENOENT`（PATH 上に無い）→ `not_found`、それ以外の例外 → `error`。review-changes 修正で3点強化: (1) 呼び出し1回あたり **5秒タイムアウト**（`WEZTERM_EXEC_TIMEOUT_MS`、`execFile` の `timeout` オプション）——ネットワーク往復を伴わないローカル/WSL interop 呼び出しのため `GithubPrPoller` の 15 秒より短め。(2) **in-flight ガード**（`private inFlight: Promise<FocusResult> | null`）——前回呼び出しが未完了なら新規 `execFile` を起動せず同じ Promise を共有し、`f` キー連打での子プロセス蓄積を防ぐ。(3) 任意の **`verifyActivation`** オプション（既定 `false`、WSL2 用インスタンスのみ `true`）——`activate-pane` の exit 0 が WSL interop 経由ではサイレント失敗しうる（upstream 未確証の懸念、`e2e-verification.md` 参照）ため、有効時は `<command> cli list --format json` で対象 pane id の実在を後追い検証し、確認できなければ `error`（`FocusService.focusWsl` の既存 Windows Terminal フォールバックへ進む）。
...
| Device / Project / Instance / Session / Event / DeviceToken / PrStatus      | domain-model  | entity (interface)                                                                                           | §7.3 DDL に対応する永続エンティティ（データのみ）。release-23 FR-02b: `Session.terminal` を追加。release-27 FR-02: `PrStatus.isDraft` を追加                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| PrStatusMapper                                                              | domain-model  | module (pr-status-mapper.ts), release-27 FR-02                                                               | `gh` CLI の生 PR 情報（state/reviewDecision/isDraft）→ `pr_status.state`/`isDraft` の写像（純粋関数、DB/`gh` 呼び出しなし）                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| SessionTerminal                                                             | domain-model  | interface (value object), release-23 FR-02b. release-28 FR-02: weztermPane 追加                              | reporter が捕捉したターミナル特定情報（`tty`/`term_program`/`tmux_pane`/`tmux_socket`/`wsl_distro`/`wt_session`/`wezterm_pane` ＋受信時刻 `seenAt`）のスナップショット。`Session.terminal`（nullable）                                                                                                                                                                                                                                                                                                                                                                                          |
...
| GithubPrPoller                                                              | hub-api       | application service (github-pr-poller.ts), release-27 FR-01/FR-01a/FR-01b                                    | `InstanceRepository.listActive()` から GitHub リモート×branch を収集し `gh` CLI で PR 状態を取得、`PrStatusMapper`/`PrStatusRepository` 経由で `pr_status` へ upsert。`gh` 未導入/未認証/稼働中の認証失効は無効化縮退（既知課題 U7 解消）                                                                                                                                                                                                                                                                                                                                                       |
...
| HubLifecycle                                                                | hub-api       | module (hub-lifecycle.ts), release-18                                                                        | pid ファイル管理（書込/削除）・生存確認＋port 疎通の3状態判定（`hubStatus`）・検証済み SIGTERM 停止（`hubStop`）。release-25-auto-update FR-01/FR-02: `probeHub` が到達性＋応答ヘッダの hub 版取得を1回で行い、`isPortReachable` はその薄いラッパーとして後方互換維持                                                                                                                                                                                                                                                                                                                           |
...
| HubApiClient / createHubConnection / HubConnection                          | cli-ink       | infrastructure / factory                                                                                     | hub への HTTP クライアントと到達先配線・再解決ファクトリ。release-25-auto-update FR-04: `getLastHubVersion()` が直近 GET 応答の `X-Monomi-Hub-Version` ヘッダを追加リクエスト無しで公開                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| HubAutostart                                                                | cli-ink       | module (hub-autostart.ts), release-18                                                                        | `monomi` 起動時の hub 自己修復自動起動（疎通確認→不在なら detached spawn→リトライ疎通待ち）。release-25-auto-update FR-02: 疎通済みなら `VersionCompare.compareVersion` で hub 版と自版を照合し、`config.yml` の `auto_update` に従って graceful 停止（`HubLifecycle.hubStop`）→新版 spawn、または版ずれ notice のみ返す。戻り値は `Promise<string \| null>`（起動 notice）に変更（破壊的変更）                                                                                                                                                                                                 |
| VersionCompare / VersionComparison                                          | cli-ink       | module (version-compare.ts), release-25-auto-update                                                          | 自版に対する他方バージョンの相対関係判定（`compareVersion`: `older`/`same`/`newer`/`unknown`）。パース不能・欠落は `unknown` として旧版と同じ更新経路に倒すポリシーを一元化。比較自体は `node-version-check.ts` の `parseVersionTriple`/`compareVersionTriples` を再利用。`HubAutostart`（hub 版照合）・`AppView`（`remoteHubNotice`）が使うほか、本図対象外の `install-hooks.ts`（`ensureReporterUpToDate`、reporter 版照合）も共有する単一ソース                                                                                                                                              |
...
| Osc8Hyperlink                                                               | cli-ink       | module (osc8-hyperlink.ts), release-27 FR-05b                                                                | PR 番号等を OSC 8 ハイパーリンクへ変換（`DetailView` の `pr` フィールド）。URL 検証（scheme/host/資格情報/パス形式/制御文字）失敗時はプレーンテキストへフォールバック                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| FocusTarget / FocusTargetOps                                                | cli-ink       | value object / module (focus/focus-target.ts), release-23. release-28-wezterm-focus FR-03a: weztermPane 追加 | 検証済みフォーカス対象。`TerminalDto`（wire）を厳格検証し不合格フィールドを個別に `null` へ縮退。`weztermPane` は `/^\d+$/`（数字のみ）で検証                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| FocusResult / Strategy                                                      | cli-ink       | type alias / interface (focus/types.ts), release-23. release-28-wezterm-focus FR-04-pre                      | フォーカス実行結果の列挙／strategy 共通インターフェイス（`matchesHint`+`focus(target)`。TerminalAppStrategy・GhosttyStrategy・WeztermFocusStrategy が実装。`focus(tty: string)`→`focus(target: FocusTarget)` の破壊的シグネチャ変更で、TTY 以外（`weztermPane`）を必要とする strategy にも対応）                                                                                                                                                                                                                                                                                                |
| TmuxSwitchOutcome / TmuxStrategy / WslStrategy                              | cli-ink       | type alias / interface (focus/types.ts), release-23                                                          | tmux 切替結果（成功時のみ外側クライアント TTY を伴う判別ユニオン）／tmux・WSL 用の専用インターフェイス（`Strategy` とは別、`matchesHint` を持たない、シグネチャ変更の影響を受けない）                                                                                                                                                                                                                                                                                                                                                                                                           |
| FocusService                                                                | cli-ink       | application service (focus/focus-service.ts), release-23. release-28-wezterm-focus FR-04 で 3→4 系統へ再設計 | ディスパッチ（tmuxPane あれば TmuxStrategy.switchClient 優先、成功時 weztermPane は null へ縮退 → tty/weztermPane 両方無ければ no_terminal → darwin は Strategy[]（WeztermFocusStrategy 含む）を term_program ヒント順に総当たり → WSL2 は weztermPane があれば weztermWslStrategy を先に試し失敗/未配線なら WslStrategy.focus へフォールバック → ネイティブ Linux は weztermPane＋weztermStrategy があればその結果、無ければ unsupported_platform。`cli.ts` は weztermStrategy を意図的に未配線のため実際は常に unsupported_platform、既知課題 U21）                                           |
| TerminalAppStrategy / GhosttyStrategy                                       | cli-ink       | strategy (focus/*.ts), release-23                                                                            | `Strategy` 実装。Terminal.app（tty 一致タブ選択+前面化）／Ghostty（OSC タイトルタグ+System Events メニュー検索、finally でタグ消去+1回リトライ）。ともに Osascript/AppleScriptOps 経由。release-24 FR-06/B12: 操作前に `exists process` で起動確認するガードを追加し、未起動なら `not_found`（Ghostty はタグ書き込み成功時のみ finally で消去）。release-28-wezterm-focus 所見対応: `focus(target)` の `target.tty === null` を自身で検証し `not_found` を返す（呼び出し側が非 null を保証する前提を廃止）                                                                                      |
...
| WeztermFocusStrategy                                                        | cli-ink       | strategy (focus/wezterm-strategy.ts), release-28-wezterm-focus FR-03b/FR-04, 新規                            | `Strategy` 実装。`<command> cli activate-pane --pane-id <weztermPane>` を `execFile` 非 shell で実行しペイン単位フォーカス。`command`（単一文字列／`ENOENT` 時フォールバック配列）差し替えで darwin・WSL2 interop の2経路に使い回す（`cli.ts` が用途別にインスタンス配線。ネイティブ Linux 向けは既知課題 U21 により未配線）。5秒タイムアウト・in-flight ガード（`f` 連打での子プロセス蓄積防止）・任意の `verifyActivation`（WSL2 用インスタンスのみ有効、`cli list --format json` で前面化を後追い検証）・任意の `raiseWindow`（activate-pane 成功後の OS レベル前面化、darwin/WSL のみ注入） |
| Osascript / AppleScriptOps / ExecFileFn                                     | cli-ink       | module / type alias (focus/osascript.ts・focus/applescript.ts), release-23                                   | `execFile` 非 shell 起動の DI（`ExecFileFn`。osascript.ts/tmux-strategy.ts/wsl-strategy.ts/wezterm-strategy.ts がそれぞれ独立宣言）＋ AppleScript 文字列エスケープ（セキュリティ三段防御の第二・第三段、`ARCHITECTURE.md` §14.3）                                                                                                                                                                                                                                                                                                                                                               |
