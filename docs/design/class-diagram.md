...
| WeztermFocusStrategy                                                        | cli-ink       | strategy (focus/wezterm-strategy.ts), release-28-wezterm-focus FR-03b/FR-04, 新規                            | `Strategy` 実装。`<command> cli activate-pane --pane-id <weztermPane>` を `execFile` 非 shell で実行しペイン単位フォーカス。`command` 差し替え（`'wezterm'`／`'wezterm.exe'`）で darwin・ネイティブ Linux・WSL2 interop の3経路に使い回す（`cli.ts` が用途別に3インスタンス配線）。5秒タイムアウト・in-flight ガード（`f` 連打での子プロセス蓄積防止）・任意の `verifyActivation`（WSL2 用インスタンスのみ有効、`cli list --format json` で前面化を後追い検証）                                            |
