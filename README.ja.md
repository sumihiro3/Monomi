...
- 任意: [`gh` CLI](https://cli.github.com/)（`gh auth login` で認証済み）を導入すると、GitHub の PR レビュー状態のポーリング（`PR_WAIT` 状態）が有効になる。未導入の場合は hub が起動時に警告を1回だけ出力し、従来の挙動にフォールバックする（詳細は後述の「GitHub PR レビュー状態」を参照）
...
`monomi` を引数なしで起動するたびに、稼働中の hub と配置済みの reporter スクリプトが実行中の CLI と同じバージョンかどうかを確認し、両者を同期させる。これにより `npm install -g monomi-cli` / `npx monomi-cli@latest` でアップグレードしても、旧バージョンのプロセスやスクリプトが取り残されない。
...
セッション行を選択した状態で `f` キーを押すと、そのセッションが動作しているターミナルウィンドウが前面に出て、対応するタブがフォーカスされる。これは、実行している CLI と同一マシン上のセッションでのみ動作する。別デバイスのセッション行を選択して `f` を押した場合は、画面にメッセージが表示されるだけで何も起きない（終了済みのセッション、ターミナル情報がないセッション、検出されたターミナルアプリが現在起動していない場合も同様）。Terminal.app・Ghostty・tmux・WezTerm に対応している。
...
**macOS** では、WezTerm フォーカスは追加設定なしで動作する。Monomi は WezTerm 公式 CLI（`wezterm cli activate-pane --pane-id <id>`）を実行したうえで WezTerm アプリケーション自体を前面化し、セッションが動いている実際のペインへフォーカスする。

> **ネイティブ Linux（X11/Wayland）は現時点で WezTerm フォーカスに未対応。** macOS での検証で、WezTerm のウィンドウを前面化するには WezTerm CLI 実行(上記)だけでなく追加の一手が必要だと判明したが、Monomi はネイティブ Linux 向けに X11/Wayland 非依存の同等手段をまだ検証できていない。未検証のまま「対応」と謳うのは避け、これが分かるまで対応を見送る（[既知課題 U21](docs/known-issues.md) 参照）。

**WSL2** では、WezTerm が Windows 側で `$WEZTERM_PANE` 環境変数を設定するが、WSL の interop レイヤーは `WSLENV` に列挙された環境変数しか WSL 側セッションへ転送しない。`$WEZTERM_PANE` を WSL2 側に届けるため、Windows 側の `.wezterm.lua` に以下を追記する。
...
> 既に `.wezterm.lua` や Windows の環境変数で `WSLENV` を設定済みの場合は、上書きせずコロン区切りで追記する（例: `WSLENV = 'EXISTING_VAR:WEZTERM_PANE'`）。
...
- **ネイティブ Linux（X11/Wayland）**: 現在未対応（上記の WezTerm の注記を参照）。
...
hub は稼働中インスタンスのブランチについて GitHub の PR レビュー状態をポーリングするようになり、ダッシュボードの「PR レビュー待ち」（`PR_WAIT`）状態が実際に発火するようになった。`github_pr_poll.interval`（既定5分）の周期ごとに、リモートが `github.com` である稼働中インスタンスから重複排除した `(project, branch)` の組を収集し、組ごとに1回 `gh pr list` を呼び出して結果を保存する。

- hub が動作するマシンに `gh` CLI がインストール済み・認証済み（`gh auth login`）である必要がある（前述「対応環境」参照）。`gh` が未導入・未認証の場合（起動時、または稼働中に認証が失効した場合を含む）、hub は警告を1回だけログ出力してポーリングを無効化し、従来の挙動（`pr` は常に none 表示）にフォールバックする。それ以外の動作には影響しない。
- 詳細ビューの `pr` フィールドには実際の状態 — `レビュー待ち` / `修正を要求` / `承認済み` / `マージ済み` / `PR なし` — が表示され、draft PR には `(ドラフト)` の注記が付く。PR 番号は対応端末（Ghostty・iTerm2 等）ではクリック可能な OSC 8 ハイパーリンクとして表示され、クリックするとブラウザで該当 PR が開く。非対応端末ではプレーンテキストで表示される。
- レビュー未着手（draft を含む）の PR のみが `PR_WAIT` としてカウントされる。修正要求・承認済み・マージ済みの PR は、ダッシュボードの他のステータスにフォールバックする。
- ポーリング対象は `github.com` リモートのみ（GitLab・Bitbucket 等は対象外）。ポーリング間隔の変更・無効化・対象リポジトリの allowlist 制限は、後述の設定テーブルの `github_pr_poll` を使う。
...
| `github_pr_poll.enabled`              | `true`                 | hub が GitHub の PR レビュー状態をポーリングするか（前述「GitHub PR レビュー状態」参照）。`false` にするとポーリングを完全に無効化する                                                                                                                               |
| `github_pr_poll.interval`             | `5m`                   | ポーリング間隔。`1m` 〜 `60m` の範囲                                                                                                                                                                                                                                 |
| `github_pr_poll.allowed_repos`        | （なし＝全リポジトリ） | ポーリング対象を制限する `owner/repo` 形式の allowlist（任意）。ペアリング済み child デバイスにより hub が意図しないリポジトリを問い合わせてしまうことを防ぐ。未設定の場合、稼働中インスタンスが報告するすべての `github.com` リモートブランチがポーリング対象になる |
...
`npx` を利用している場合は `npx monomi-cli@latest` を実行してください。

アップグレード後、次回 `monomi` を引数なしで起動すると、稼働中の hub と配置済みの reporter スクリプトが CLI の新しいバージョンと自動的に同期されます（前述「自動アップデート（hub・reporter）」を参照）。hub・reporter 自体はすぐに更新されず、次回の引数なし起動時に更新されます。

## アンインストール

以下の順で行う（DB・token を含む `~/.monomi` は最後にまとめて削除する）。

このマシンで launchd により hub を常駐化している場合（前述「hub の起動と常駐化」で紹介した `~/Library/LaunchAgents/com.monomi.hub.plist` の LaunchAgent）は、下記の手順3の前に `launchctl unload ~/Library/LaunchAgents/com.monomi.hub.plist` を手動で実行すること。`monomi hub stop` は hub プロセスへ `SIGTERM` を送るだけで、launchd には一切関与しない。この LaunchAgent は `KeepAlive` が `true` のため、`SIGTERM` の直後に launchd が hub を自動的に再起動してしまう。手動での `launchctl unload` を省略すると、「止まった」と誤認したまま hub が裏で稼働し続けた状態のまま `npm uninstall -g` / `rm -rf ~/.monomi` へ進んでしまうおそれがある。plist ファイル自体の削除（手順1）も忘れないこと。残したままだと `RunAtLoad`／`KeepAlive` が `true` のまま残り、削除済みの `monomi` バイナリを launchd がログインのたびに起動しようとし続ける。

```sh
launchctl unload ~/Library/LaunchAgents/com.monomi.hub.plist  # 0. launchd で常駐化していた場合のみ: 先に自動再起動を無効化
rm -f ~/Library/LaunchAgents/com.monomi.hub.plist              # 1. launchd で常駐化していた場合のみ: LaunchAgent の plist ファイルを削除
monomi uninstall-hooks     # 2. Claude Code の settings.json から Monomi 起因のフックのみ除去
monomi hub stop            # 3. 稼働中の hub を停止（SIGTERM・終了確認後 pid ファイル削除）
npm uninstall -g monomi-cli  # 4. グローバルパッケージを削除（グローバルインストールしていた場合のみ）
rm -rf ~/.monomi           # 5. config.yml・SQLite DB・token・reporter を含め全データを削除
```
