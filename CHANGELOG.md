...
- 新キー `f` によるターミナルフォーカスが WezTerm に対応（対応ターミナルは Terminal.app・Ghostty・tmux・WezTerm の4つに拡大）。macOS では追加設定なしで WezTerm 公式 CLI によるペイン単位フォーカスが動作する
...
- README／README.ja.md に「WezTerm: ペイン単位フォーカス」節を新設。有効化に必要な設定手順（WSL2 の `WSLENV` 追記を含む）と、macOS・WSL2 それぞれの対応範囲を明記
...
- ネイティブ Linux（X11/Wayland）向けの WezTerm フォーカス対応を撤回。実機検証で、WezTerm ウィンドウの前面化には `wezterm cli activate-pane` だけでなく OS ごとの追加手順が必要と判明したが、ネイティブ Linux 向けの検証ができなかったため、未検証のまま「対応」と謳うことを避けた（既知課題 U21）
...
- `~/.monomi/config.yml` に新設定ブロック `github_pr_poll`（`enabled`／`interval`／`allowed_repos`）を追加。未設定の場合は既定値（有効・5分間隔・allowlist なし）で後方互換動作する
