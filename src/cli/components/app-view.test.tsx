...
describe('AppView — f キーでのフォーカス実行・device_id ゲート・notice（release-23-terminal-focus FR-05c）', () => {
  ...
  it('release-28-wezterm-focus 実機検証対応: tty が検証不合格でも wezterm_pane が有効なら noTerminalInfo にならず focusRunner を呼ぶ', async () => {
    const row = makeRow({
      id: '1',
      projectName: 'ProjectLens',
      display: 'active',
      priority: 1,
      // WSL2 実機で確認された症状の再現: resolve_tty() が不正な値（`/dev/?`）を返し tty 検証は
      // null へ縮退するが、wezterm_pane は有効な数字。以前はこのケースでも app-view.tsx の事前
      // ゲートが tty/tmuxPane のみを見て noTerminalInfo に縮退させ、focusRunner を呼んでいなかった。
      terminal: validTerminal({ tty: '/dev/?', wezterm_pane: '0' }),
    })
    const fake = new FakeHubApiClient([row])
    const focusRunner = vi.fn<FocusRunner>(async () => 'ok')
    const { lastFrame, stdin } = renderAppFocus(fake, { focusRunner })
    ...
    await vi.waitFor(() => expect(focusRunner).toHaveBeenCalledTimes(1))
    expect(focusRunner).toHaveBeenCalledWith(
      expect.objectContaining({ tty: null, tmuxPane: null, weztermPane: '0' })
    )
    await new Promise((resolve) => setTimeout(resolve, 30))
    expect(lastFrame()).not.toContain(t('focus.noTerminalInfo'))
  })
