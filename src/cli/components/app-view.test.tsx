...
describe('AppView — 詳細ビュー中のショートカットヒント切替と無効化（FR-04）', () => {
  it('一覧表示中と詳細表示中でフッターのヒントが切り替わる', async () => {
    ...
    stdin.write('\r') // Enter → 詳細
    await vi.waitFor(() => expect(lastFrame()).toContain('イベント履歴'))

    // 詳細表示中: 詳細固有ヒント（j/k scroll・←/→ project・w wrap）へ切り替わる（release-6 FR-03 AC-1）。
    // 一覧向けの 1-5 filter・↵ detail は出ない。j/k は本リリースでイベントスクロールへ再割当てされた
  ...
  it('詳細表示中もフィルタ（1-5）は無視され、j/k は一覧状態に影響しない（イベントスクロールへ再割当て）', async () => {
    ...
    stdin.write('j')
    await new Promise((resolve) => setTimeout(resolve, 20))
    expect(lastFrame()).toContain('イベント履歴') // 詳細表示のまま（j は詳細内スクロール、遷移しない）

    stdin.write(String.fromCharCode(27)) // esc で一覧へ戻る
    // 'ProjectLens' は詳細ビューにも出るため判定に使わない。詳細固有の BOX タイトル 'イベント履歴'
    // の不在で「一覧へ戻った」ことを判定する。
    await vi.waitFor(() => expect(lastFrame()).not.toContain('イベント履歴'))
...
describe('AppView — 詳細ビューから隣接プロジェクトへの移動（release-6 FR-04）', () => {
  ...
  it('AC-3: 隣接移動後に esc で戻ると一覧カーソルが移動先 instance に一致する（selectedIndex 一本化）', async () => {
    ...
    stdin.write(ESC) // 一覧へ戻る（詳細固有の BOX タイトル 'イベント履歴' の不在で判定）
    await vi.waitFor(() => expect(lastFrame()).not.toContain('イベント履歴'))
...
describe('AppView — 閲覧中 instance が一覧から消えたときの復帰（review-changes 修正）', () => {
  it('他の instance が残っていても、閲覧対象そのものが消えたら一覧へ戻る（隣へ無言で切り替わらない）', async () => {
    const rowA = makeRow({ id: '1', projectName: 'ProjectLens', display: 'active', priority: 1 })
    const rowB = makeRow({
      id: '2',
      projectName: 'Monban',
    ...
    // ポーリングで ProjectLens が一覧から消え、Monban だけが残る（index 0 に Monban がスライド
    // してくるため、selectedRow は undefined にならない）。
    fake.instances = [rowB]
    await vi.waitFor(
      () => {
        const frame = lastFrame() ?? ''
        // 一覧へ戻っていること（詳細固有の見出し「イベント履歴」が消える）。
        expect(frame).not.toContain('イベント履歴')
      },
      { timeout: 2000 }
    )
    // 隣（Monban）の詳細へ無言で切り替わってはいない。一覧に戻り Monban がカードとして見える。
    expect(lastFrame()).toContain('Monban')
  })
})
...
describe('AppView — 詳細ビュー表示中にヘルプを開いても画面があふれない（review-changes 修正）', () => {
  ...
  it('review-changes 修正: detail 表示中に ? でヘルプを開くと、DetailView の表示件数（visible）がヘルプ分だけ正しく減る', async () => {
    ...
    // rows=30 → DETAIL_RESERVED_ROWS(17) を引いた visible=13。30 件あれば範囲ラベルの変化を
    // 確実に観測できる（13件未満だとヘルプなしで既に全件表示になり、差が出ない）。
    ...
    stdin.write('\r')
    // ヘルプなし: visible=13 → 最下部は "18-30 of 30"。
    await vi.waitFor(() => expect(composedFrame()).toContain('18-30 of 30'))

    stdin.write('?')
    await vi.waitFor(() => expect(composedFrame()).toContain('キーバインド'))
    // ヘルプあり: extraReservedRows が配線されていれば visible が縮み、
    // 範囲ラベルが "18-30 of 30" とは異なる（より狭い）ものへ変わるはず。
    await vi.waitFor(() => {
      const frame = composedFrame()
      expect(frame).toContain('キーバインド')
      expect(frame).not.toMatch(/18-30 of 30/)
    })
