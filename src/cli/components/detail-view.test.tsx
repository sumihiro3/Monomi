...
/**
 * `dimColor` 等が付与する ANSI エスケープシーケンスを取り除く（terminal 行の 1 行単位の
 * 正規表現マッチで、`Field` の `label` に付与される dimColor のコード列に邪魔されないようにする）。
 */
...
describe('DetailView — terminal 行（release-24 FR-02、U16）', () => {
  it('AC-1/AC-2: session.terminal.term_program が既知値なら terminalDisplayName の表示名を terminal 行に表示する', async () => {
  ...
  it('AC-2: session.terminal が null（旧 reporter・情報未捕捉）の場合は他の nullable フィールドと同じ "-" を表示する', async () => {
    // makeRow() の既定は session.terminal: null（互換性: 非機能要件）。
    const row = makeRow()
    const fake = new FakeDetailClient(() => makeDetail(row, []))
    const { lastFrame } = renderDetail(fake, row, 30)
    ...
    // terminal 行が "Terminal      -" の形で描画される（Field の label.padEnd(12) + '-'）。
    // label は dimColor（ANSI エスケープ付き）で描かれるため、比較前に取り除く。
    const frame = stripAnsi(lastFrame() ?? '')
    const terminalLine = frame.split('\n').find((line) => line.includes('Terminal'))
    expect(terminalLine).toBeDefined()
    expect(terminalLine ?? '').toMatch(/Terminal\s+-/)
  })

  it('AC-4: tmux アタッチ中（term_program==="tmux"）は "tmux" を表示する', async () => {
    const row: InstanceStatusRow = {
      ...makeRow(),
      session: {
        id: 'sess-1',
    ...
    await vi.waitFor(() => {
      const frame = stripAnsi(lastFrame() ?? '')
      const terminalLine = frame.split('\n').find((line) => line.includes('Terminal'))
      expect(terminalLine ?? '').toMatch(/Terminal\s+tmux/)
    })
  })
})
...
describe('DetailView — 実TTY経路（isTTY=true・columns/rows 取得可）でのBOX罫線と可変表示行数（FR-02 AC-5 / FR-06 / FR-07）', () => {
  it('rows 由来の可変表示行数・自前タイトル/範囲ラベル罫線・角整列（borderTop=false）を実TTY相当で確定させる', async () => {
    ...
    // その経路を直接踏み、(1) 表示行数が rows から算出される（visible = 30 − DETAIL_RESERVED_ROWS(18) = 12
    // → 15 件の最下部は 4-15 of 15。固定フォールバック 10 行なら 6-15 になるので値の差で経路を判別できる）、
    ...
    // FR-02 AC-5: 表示行数は rows(30) から算出（visible=12）。固定フォールバック 10 行なら 6-15 になる。
    expect(screen).toContain('4-15 of 15')
    expect(screen).not.toContain('6-15 of 15')
    // FR-07: 範囲ラベルはイベント履歴 BOX の下辺罫線（╰ … ╯）へ右寄せで埋め込まれる。
    expect(screen).toMatch(/╰.*4-15 of 15.*╯/)
...
describe('DetailView — 端末リサイズ時の tail-follow 再ピン留め（review-changes 修正）', () => {
  it('最下部表示中に rows が縮小しても、新しい最下部へ追従し直す（新着追従が止まらない）', async () => {
    ...
    const row = makeRow()
    // rows=30 → visible=12（DETAIL_RESERVED_ROWS=18）。34 件で最下部は "23-34 of 34"。
    ...
    // 初回ロード後、最下部（23-34 of 34）で安定するのを待つ。
    await vi.waitFor(() => expect(lastScreen()).toContain('23-34 of 34'))
...
describe('DetailView — 折り返しモードでの表示件数の動的な絞り込み（release-6 FR-10）', () => {
  it('AC-1/AC-2: 折り返しモードでは各イベントが複数行に跨る分、表示件数が切り詰めモードより減り、範囲ラベルが実表示件数を反映する', async () => {
    ...
    const row = makeRow()
    // rows=27 → visible=9（DETAIL_RESERVED_ROWS=18）。15件あり、切り詰めモードなら最下部は
    // "7-15 of 15"（9件）になる。折り返しモードでは各イベントの長い tool_summary が複数行に
    // 跨るため、9件は収まりきらず表示件数が減るはず。
    ...
    // 切り詰めモード（既定）: FR-02 の従来どおり最下部 "7-15 of 15"（9件）を表示する。
    await vi.waitFor(() => expect(lastScreen()).toContain('7-15 of 15'))

    // FR-08: w で折り返し ON。FR-10: 各イベントが複数行に跨るため、9件は visible(9) に
    // 収まりきらず、表示件数が減った範囲ラベルになる（"7-15 of 15" ではなくなる）。Z(=15) は不変。
    stdin.write('w')
    await vi.waitFor(() => {
      const screen = lastScreen()
      expect(screen).toMatch(/\d+-15 of 15/)
      expect(screen).not.toContain('7-15 of 15')
    })
    ...
    // 9件（7-15）より狭い範囲（開始 index が 7 より大きい = 表示件数が減っている）になっているはず。
    expect(wrappedStart).toBeGreaterThan(7)

    // FR-10 AC-6: 折り返しモードでもヘッダー・概要BOXは固定表示のままスクロールして消えない
    // （画面があふれていない = イベント履歴BOXの内容が visible(9) 行に収まっている証跡として、
...
describe('DetailView — 埋め込み改行を含むイベントでの表示件数絞り込み（review-changes 修正）', () => {
  ...
  it('review-changes 修正（ユーザー指摘）: 境界イベントが丸ごとは収まらない場合、末尾側（より新しい部分）だけ部分採用してBOXを埋め切る', async () => {
    ...
    const row = makeRow()
    // 12行の段落（各行は幅に十分収まる短さ）を持つ最新イベント1件のみ。visible=7 のため
    // 丸ごとは収まらず、末尾7行（paragraph line 6〜12）だけが残るはず。
    const multiLineSummary = Array.from({ length: 12 }, (_, i) => `paragraph line ${i + 1}`).join(
      '\n'
    )
    const events = [
    ...
    // 末尾（より新しい）7行だけが残り、BOX がちょうど埋まる。先頭5行（プレフィックス込みの
    // 1行目〜5行目）は間引かれて表示されない。
    // "line 1" は "line 10/11/12" の前方一致になるため、直後が数字でないことを正規表現で確認する。
    expect(screen).toContain('paragraph line 6')
    expect(screen).toContain('paragraph line 12')
    expect(screen).not.toMatch(/paragraph line 1\D/)
    expect(screen).not.toContain('paragraph line 5')
