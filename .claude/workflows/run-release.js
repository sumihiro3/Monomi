export const meta = {
  ...
  phases: [
    ...
    { title: '実装', detail: 'implement-feature の実行と Gate0.5(完了性・捏造疑いの照合)' },
    ...
    { title: '起票', detail: '未対応所見・収束不能分の known-issues 起票' },
    { title: '文書同期', detail: 'sync-docs と最終 release-check(別枠修正ループつき)' },
 ...
 * @returns {fixed, guard} — fixed は修正エージェントが結果を返したか。
 */
...
for (let iter = 0; ; iter++) {
  ...
  const review = await workflow('review-changes', { base: baseBranch, config })
  addConsumption(
    'gate2',
    review
      ? Math.max(
  ...
  const fixText = toFix
    ...
    .join('\n')
  await fixWithGuard(`以下のレビュー所見を修正してください。\n${fixText}`, 'Gate2 レビューループ', 'gate2')
...
const finalGate = await gate1Loop(2, '文書同期', 'finalCheck', '最終検査')
