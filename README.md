...
CLI の表示言語は既定で English。日本語表示にするには `locale: ja` を明示的に設定するか、OS の `LANG` 環境変数から自動判定させる（旧バージョンから引き継ぐ既存ユーザーは、日本語表示を維持するためにこの設定の追記が必要）。
...
| `locale`                              | （未設定時に自動判定） | CLI の表示言語。`ja` または `en`。未設定時は OS の `LANG` 環境変数から自動判定し、判定できない場合は `en` にフォールバック |
