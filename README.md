...
- 必要 Node.js バージョン: `>=22.5.0`（`package.json` の `engines.node` と一致）
...
グローバルインストール後、`monomi` コマンドが使えるようになる。
...
hub は端末を閉じても動き続けてほしい常駐プロセスなので、[pm2](https://pm2.keymetrics.io/) での常駐化を推奨する（実機で動作確認済み）。

```sh
pm2 start monomi --name monomi-hub -- hub   # monomi hub を pm2 管理下で起動
pm2 save                                     # 現在のプロセス構成を保存
pm2 startup                                  # OS 起動時に pm2 自身を自動起動する場合（初回のみ。表示されたコマンドを実行）
```
...
pm2 delete monomi-hub && pm2 save    # 2. pm2 で常駐化していた場合、hub プロセスを停止・削除（該当する場合のみ）
npm uninstall -g monomi-cli          # 3. グローバルパッケージを削除
